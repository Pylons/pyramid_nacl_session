...
class TestEncryptedCookieSessionFactory(unittest.TestCase):
    ...
    def test_cookie(self):
        ...
        test_secure_prefix = self._build_cookie({'pynacl.session.secure':
                                                'true'})
        test_not_secure = self._build_cookie({'session.secure': 'false'})
        test_http_only = self._build_cookie({'session.httponly': 'true'})
        test_http_secure = self._build_cookie({'pynacl.session.secure': 'true',
                                              'session.httponly': 'true'})
        test_cookie_name = self._build_cookie({'session.cookie_name':
                                               '_test_'})
